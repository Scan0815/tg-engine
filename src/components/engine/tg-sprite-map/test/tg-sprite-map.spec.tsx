    ...
    it('renders with default props', async () => {
      ...
      expect(page.root).toEqualHtml(`
        <tg-sprite-map style="width: 80px; height: 80px;">
          <mock:shadow-root>
            <canvas class="static-layer" height="80" width="80"></canvas>
            <canvas class="animated-layer" height="80" width="80"></canvas>
          </mock:shadow-root>
        </tg-sprite-map>
      `);
    });
    ...
    it('renders with custom dimensions', async () => {
      ...
      expect(page.root).toEqualHtml(`
        <tg-sprite-map style="width: 320px; height: 320px;">
          <mock:shadow-root>
            <canvas class="static-layer" height="320" width="320"></canvas>
            <canvas class="animated-layer" height="320" width="320"></canvas>
          </mock:shadow-root>
        </tg-sprite-map>
      `);
    });
    ...
    it('renders with scale', async () => {
      ...
      expect(page.root).toEqualHtml(`
        <tg-sprite-map style="width: 160px; height: 160px;">
          <mock:shadow-root>
            <canvas class="static-layer" height="160" width="160"></canvas>
            <canvas class="animated-layer" height="160" width="160"></canvas>
          </mock:shadow-root>
        </tg-sprite-map>
      `);
    });
  ...
  describe('dom mode', () => {
    it('renders with default props', async () => {
      const page = await newSpecPage({
        components: [TgSpriteMap],
        template: () => (
          <tg-sprite-map renderMode="dom"></tg-sprite-map>
        ),
      });
      expect(page.root).toEqualHtml(`
        <tg-sprite-map style="width: 80px; height: 80px;">
    ...
    it('handles empty tiles array', async () => {
      const page = await newSpecPage({
        components: [TgSpriteMap],
        template: () => (
          <tg-sprite-map tiles={[]} renderMode="dom"></tg-sprite-map>
        ),
      });
